...
type Row = {
  title: string;
  slug: string;
  content: string; // محتوا به صورت HTML ذخیره شده
...
/** حذف تگ‌های HTML برای ساخت خلاصهٔ تمیز */
        ...
        <ul className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {currentPosts.map((p) => {
            ...
            return (
              <li
                key={p.slug}
                className="rounded-xl bg-white shadow-sm  overflow-hidden hover:shadow-md transition"
              >
                {/* کاور */}
                {p.coverUrl ? (
