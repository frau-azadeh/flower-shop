            ...
            <ul className="space-y-3">
                {posts.map((p)=>(
                    <li key={p.slug} className="rounded-md border p-3">
                        <a href={`/blog/${p.slug}`} className="font-bold text-primary">{p.title}</a>
                        <div className="text-xs text-slate-300">{p.publishedAt ? new Date(p.publishedAt).toLocaleDateString(): ""}</div>
                    </li>
                ))}
            </ul>
        </main>
