...
type OrderStatus = "pending" | "processing" | "delivered" | "canceled" | "returned";
...
const CONTENT: Record<StatusTab, { icon: ReactNode; text: string }> = {
  all:        { icon: <Flower2 className="size-24 text-slate-300" />,          text: "هنوز سفارشی ثبت نشده است." },
  pending:    { icon: <Clock className="size-24 text-slate-300" />,         text: "سفارش در انتظاری وجود ندارد." },
  processing: { icon: <Loader2 className="size-24 text-slate-300" />,        text: "سفارشی در حال پردازش نیست." },
  delivered:  { icon: <PackageCheck className="size-24 text-slate-300" />, text: "سفارش تحویل‌شده‌ای یافت نشد." },
  canceled:   { icon: <XCircle className="size-24 text-slate-300" />,        text: "سفارشی لغو نشده است." },
  returned:   { icon: <Undo2 className="size-24 text-slate-300" />,            text: "مرجوعی ثبت نشده است." },
};
...
export default function OrdersCard() {
  ...
  // وضعیت فعلی از URL
  const STATUSES: StatusTab[] = ["all","pending","processing","delivered","canceled","returned"];
  ...
  return (
    <section dir="rtl" className="mx-auto max-w-6xl p-4 md:p-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-6 text-right">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2 text-sm" role="tablist" aria-label="وضعیت سفارش">
          {TABS.map((t) => {
