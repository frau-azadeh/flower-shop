...
export default function HeaderCartIcon({ className = "" }: { className?: string }) {
  const [hydrated, setHydrated] = useState(false);
  useEffect(() => setHydrated(true), []);
  const totalQty = useAppSelector(selectCartTotalQty);

  return (
    <Link href="/cart" className={`relative p-2 rounded-full hover:bg-muted ${className}`} aria-label="سبد خرید">
