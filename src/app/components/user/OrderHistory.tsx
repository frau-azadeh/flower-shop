...
const baseTabs: TabItem[] = [
  { key: "current",   label: "جاری",       icon: <Clock className="size-4" />,       emptyIcon: <Clock className="size-24 text-slate-300" />,       emptyText: "سفارشی در حال انجام ندارید",       badgeCount: 0 },
  { key: "delivered", label: "تحویل شده",  icon: <PackageCheck className="size-4" />, emptyIcon: <PackageCheck className="size-24 text-slate-300" />, emptyText: "سفارش تحویل شده ای وجود ندارد",   badgeCount: 0 },
  { key: "returned",  label: "مرجوع شده",  icon: <Undo2 className="size-4" />,       emptyIcon: <Undo2 className="size-24 text-slate-300" />,       emptyText: "مورد مرجوع شده ای ندارید",        badgeCount: 0 },
  { key: "canceled",  label: "لغو شده",    icon: <XCircle className="size-4" />,     emptyIcon: <XCircle className="size-24 text-slate-300" />,     emptyText: "سفارشی لغو نشده است",             badgeCount: 0 },
];
...
export default function OrderHistory() {
  ...
  const tabs = useMemo<TabItem[]>(
    () => baseTabs.map((t) => ({ ...t, badgeCount: counts[t.key as keyof typeof counts] ?? 0 })),
    [counts]
  );
    ...
    () =>
      visible.filter((o) =>
        [o.id, o.fullName, o.phone, o.address].some((f) =>
          String(f ?? "").toLowerCase().includes(query.toLowerCase())
        )
      ),
    [visible, query]
  );
            ...
            {filtered.map((o) => (
              <div key={o.id} className="border rounded-2xl p-4">
                ...
                <div className="mt-2 text-sm text-slate-700">
                  ...
                  <div className="mt-1 font-medium">مبلغ: {o.grandTotal.toLocaleString()} تومان</div>
