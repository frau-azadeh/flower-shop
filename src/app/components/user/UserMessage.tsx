...
const CONTENT : Record<Tab, {text:string, icon: ReactNode}> = {
    inbox: {text: "هنوز پیامی در صندوق شما نیست", icon: <Inbox className='size-24 text-slate-300'/>},
    unread: {text:"پیام خوانده نشده ای ندارید", icon: <Mail className='size-24 text-slate-300'/>},
    starred: {text: "پیام ستاره دار ثبت نشده است", icon: <Star className='size-24 text-slate-300'/>},
    archive: {text:"پیامی در بایگانی وحود ندارد", icon: <Archive className='size-24 text-slate-300'/>},
};
...
const UserMessage:React.FC = () => {
  ...
  <section className='rounded-2xl border border-slate-200 bg-white p-4 md:p-6 text-right'>
    <div className='mb-3 flex gap-2 ' role='tablist' aria-label='تب های پیام ها'>
        {TABS.map((t)=>{
